...
var _ = Describe("Strategy controller", func() {
	It("still reconciles a spec change (A5 predicate modernization)", func() {
		...
		key := types.NamespacedName{Name: strategy.Name, Namespace: strategy.Namespace}

		Eventually(func(g Gomega) {
			var latest freqtradev1alpha1.Strategy
		...
		var toUpdate freqtradev1alpha1.Strategy
		Expect(k8sClient.Get(ctx, key, &toUpdate)).To(Succeed())
		toUpdate.Spec.Script = "class SampleStrategy(IStrategy):\n    stoploss = -0.1"

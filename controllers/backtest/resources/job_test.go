...
// testNamespace is shared across this package's *_test.go files.
...
func TestBuildJob_NameMatchesBacktestNameExactly(t *testing.T) {
	...
	job := BuildJob(backtest, "img", "operator-img", "SampleStrategy", "my-run-config", "my-run-strategy")
	...
	if job.Namespace != testNamespace {
		t.Errorf("expected namespace %s, got %s", testNamespace, job.Namespace)
	}
}
...
func TestBuildJob_DefaultTTLWhenUnset(t *testing.T) {
	...
	job := BuildJob(backtest, "img", "operator-img", "SampleStrategy", "cfg", "strategy-cm")
